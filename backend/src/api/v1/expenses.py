...
async def create_expense(data: ExpenseCreate, service: ExpenseServiceDep) -> dict:
...
async def bulk_delete(data: BulkDeleteRequest, service: ExpenseServiceDep) -> None:
    service.bulk_delete_expenses(data.ids)
    return None


@router.post("/export", summary="Export expenses")
async def export_expenses(service: ExpenseServiceDep) -> dict:
...
async def import_expenses(data: ImportRequest, service: ExpenseServiceDep) -> dict:
...
async def get_categories(service: ExpenseServiceDep) -> dict:
    categories = service.get_categories()
    return {"status": "success", "data": categories}


@router.post("/categories", status_code=status.HTTP_201_CREATED, summary="Add category")
async def add_category(data: dict, service: ExpenseServiceDep) -> dict:
...
async def remove_category(category_name: str, service: ExpenseServiceDep) -> dict:
...
async def get_settings(service: ExpenseServiceDep) -> dict:
    settings = service.get_settings()
    return {"status": "success", "data": settings}


@router.patch("/settings", summary="Update settings")
async def update_settings(data: dict, service: ExpenseServiceDep) -> dict:
...
async def get_expense(expense_id: str, service: ExpenseServiceDep) -> dict:
...
async def update_expense(expense_id: str, data: ExpenseUpdate, service: ExpenseServiceDep) -> dict:
